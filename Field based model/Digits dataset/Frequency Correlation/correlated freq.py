...
plt.subplot(3,1,1)
plt.plot(freqs, fft.fft(time_domain_waveform))
plt.title("Process of applying the gaussian envelope")
plt.ylabel("Amplitude")

plt.subplot(3,1,2)
plt.plot(freqs, noisy_gaussian_envelope)
plt.ylabel("Amplitude")

plt.subplot(3,1,3)
plt.plot(freqs, fft.fft(time_domain_waveform) * noisy_gaussian_envelope)
plt.xlabel("Frequency(w)")
plt.ylabel("Amplitude")
plt.tight_layout()
plt.savefig("Procession of applying frequency envelope")
plt.clf()
...
# ----- Dataset Scaling -----
scale_factor = 5  # Let's replicate each image n times (adjust this to your needs)
...
plt.savefig(str("Freq Correlated Confusion Matrix"))
plt.clf()
...
plt.grid(True)
plt.savefig(str("Freq Correlated Training vs Epoch"))
